...
vkey_intr(void *arg)
{
	...
	for (nprocessed = 0; !failed; nprocessed++) {
		...
		volatile struct vkey_comp *comp = sc->sc_dma.comp.as.comps + h;
		if (comp->owner != HOST) {
